...
#include "bsp_lcd_io_psram_spi.h"    // bsp_lcd_new_panel_io_spi prototype

// Copied verbatim from esp_lcd/priv_include/esp_lcd_common.h (IDF 5.5.4) — the only
// symbol this file used from that private (non-reachable) header. Re-sync on IDF upgrade.
...
static const char *TAG = "bsp_lcd.io.spi";
...
esp_err_t bsp_lcd_new_panel_io_spi(esp_lcd_spi_bus_handle_t bus, const esp_lcd_panel_io_spi_config_t *io_config, esp_lcd_panel_io_handle_t *ret_io)
{
#if CONFIG_LCD_ENABLE_DEBUG_LOG
...
static esp_err_t panel_io_spi_tx_color(esp_lcd_panel_io_t *io, int lcd_cmd, const void *color, size_t color_size)
{
    ...
    do {
        ...
        // S3Watch fix: let GDMA read the color data straight from PSRAM instead of
        // forcing spi_master to malloc a per-flush INTERNAL bounce buffer. Aligned
        // PSRAM DMAs directly; unaligned falls back to a PSRAM-allocated bounce.
        // Either way the flush uses zero internal RAM. (Coupled to IDF 5.5.4
        // esp_lcd_panel_io_spi.c — re-sync this file on IDF upgrade.)
